...
public class Match3 : MonoBehaviour
{
    ...
    public void SetLevelSO(LevelSO levelSO)
    {
        ...
        for (int x = 0; x < gridWidth; x++)
        {
            for (int y = 0; y < gridHeight; y++)
            {
                ...
                grid.GetGridObject(x, y).SetIsHole(levelGridPosition.isHole);
            }
        }
    ...
    {
        moveCount--;
        Debug.Log("Current move count: " + moveCount);
    ...
    public void SpawnNewMissingGridPositions()
    {
        for (int x = 0; x < gridWidth; x++)
        {
            for (int y = 0; y < gridHeight; y++)
            {
                ...
                if (gemGridPosition.IsEmpty() && !gemGridPosition.IsHole())
                {
            ...
            for (int y = 0; y < gridHeight; y++)
            {
                ...
                if (!gemGridPosition.IsEmpty() && !gemGridPosition.IsHole())
                {
                    ...
                    for (int i = y - 1; i >= 0; i--)
                    {
                        ...
                        if (nextGemGridPosition.IsEmpty() && !nextGemGridPosition.IsHole())
                        {
    ...
    {
        if (x < 0 || y < 0 ||
            x >= gridWidth || y >= gridHeight || grid.GetGridObject(x, y).IsHole()) 
        {
            // Invalid position
            return false;
        }
