...
public class Match3 : MonoBehaviour
{
    ...
    public void SetLevelSO(LevelSO levelSO)
    {
        ...
        for (int x = 0; x < gridWidth; x++)
        {
            for (int y = 0; y < gridHeight; y++)
            {
                ...
                {
                    // No LevelGridPosition is found with the given x, y.
    ...
    public void SwapGridPositions(int startX, int startY, int endX, int endY)
    {
        ...
        GemGridPosition endGemGridPosition = grid.GetGridObject(endX, startY);
    ...
    public void FallGemsIntoEmptyPositions()
    {
        for (int x = 0; x < gridWidth; x++)
        {
            for (int y = 0; y < gridHeight; y++)
            {
                ...
                if (!gemGridPosition.IsEmpty())
                {
                    // Grid Position has Gem, not empty
                    for (int i = y - 1; i >= 0; i--)
                    {
    ...
    public List<GemGridPosition> GetMatch3Links(int x, int y)
    {
        ...
        int horizontalLinkAmount = 1 + leftLinkAmount + rightLinkAmount; // This Gem itself + left + right

        if (horizontalLinkAmount >= 3)
        {
            // Has at least 3 horizontal linked gems
        ...
        for (int i = 1; i < gridHeight; i++)
        {
            ...
            {
                GemSO nextGemSO = GetGemSO(x, y + 1);
                if (nextGemSO == gemSO)
                {
                    // Same Gem
                    upLinkAmount++;
        ...
        int verticalLinkAmount = 1 + downLinkAmount + upLinkAmount; // This Gem itself + down + up

        if (verticalLinkAmount >= 3)
        {
            // Has at least 3 vertical linked gems
            List<GemGridPosition> linkedGemGridPositionlist = new List<GemGridPosition>();
                    ...
                    else
                    {
                        ...
                        foreach (List<GemGridPosition> tmpLinkedGemGridPositionlist in allLinkedGemGridPositionList)
                        {
                            if (linkedGemGridPositionList.Count == tmpLinkedGemGridPositionlist.Count)
                            {
                                ...
                                if (allTheSame)
                                {
                                    // Nodes are all same, not a new unique link
                                    uniqueNewLink = false;
                                }
                            }
                        }
    ...
    {
        // Function checks both failure and succes.
        if(!HasMoveAvailable())
        {
